...
def main():
    ...
    def get_idx(dataset, minority='black', majority='white', alpha=0.5):
        ...
        if alpha is None:
            ...
            train_idx = np.concatenate([train_idx, \
                    np.random.choice(majority_idx, size=len(minority_idx)-len(train_idx), replace=False)])
        else:
            train_idx = np.random.choice(minority_idx, size=int(len(minority_idx) * alpha), replace=False)
            train_idx = np.concatenate([train_idx, \
                    np.random.choice(majority_idx, size=len(minority_idx)-len(train_idx), replace=False)])
        return train_idx

    train_dataset = datasets['train']['dataset']
    train_idx = get_idx(train_dataset, minority=config.minority, majority=config.majority, alpha=config.alpha/100)
    print('Training data: %d' % len(train_idx))

    valid_dataset = datasets['val']['dataset']
    valid_idx = get_idx(valid_dataset, minority=config.minority, majority=config.majority, alpha=None)
    print('Valid data: %d' % len(valid_idx))
    print('Alpha: %d' % config.alpha)
    ...
    results.save(os.path.join(config.log_dir, '%s_%d_%s_%s_%s.jsonl' % (config.dataset, config.alpha, config.exp_id, config.minority, config.majority)))
